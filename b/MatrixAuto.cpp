...
void saveResult(int type, int size, int cores, string time, long long values[], double consumed)
{
    ...
    line += ',' + to_string(consumed);

    std::ofstream file("results2.txt", std::ios::app);
...
int caller(int type, int size, int cores)
{
    ...
    double consumed = (energyAfter - energyBefore) / 1e6;
    saveResult(type, size, cores, time, values, consumed);
...
int main(int argc, char *argv[])
{
    ...
    vector<int> size3 = {1000, 1400, 2000};
